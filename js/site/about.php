...
class About extends MY_Controller {
	...
	public function display_user_list(){
		if ($this->checkLogin('A') == ''){
			...
			$this->data['heading'] = 'Guide List';
			$condition = array('group'=>'Guide');
	...
	public function display_user_dashboard(){
		if ($this->checkLogin('A') == ''){
			...
			$this->data['heading'] = 'Guide Dashboard';
	...
	public function add_user_form(){
		if ($this->checkLogin('A') == ''){
			...
			$this->data['heading'] = 'Add New Guide';
	...
	public function insertEditUser(){
		if ($this->checkLogin('A') == ''){
			...
			$excludeArr = array("user_id","thumbnail","new_password","confirm_password","group","status","videolink","hiddenvideo");
			...
			$inputArr = array('group' => 'Guide', 'status' => $user_status);
		  ...
		  if(!empty($_FILES["file1"]["name"])) {
					  ...
					  $is_valid_type  = (in_array($_FILES['file1']['type'], $allowedTypes))?true:false;
					   if ( $is_valid_type&& ($_FILES["file1"]["size"] < 2000000))
					  {
						...
						if (file_exists("./images/uservideo/" . $fileName))
						  {
						  ...
						  "./images/uservideo/" . time().$_FILES["file1"]["name"]);
						  //echo "Stored in: " . "./images/expvideo/" . $_FILES["file1"]["name"];
			  ...
			  //echo "File Name is Empty"; die;
			   $videofilename = $this->input->post('hiddenvideo');
		  }
			...
			$dataArr = array("videolink"=>$videofilename);
			
			$dataArr = array_merge($inputArr,$user_data,$dataArr);
	...
	{	
		...
		//$Query = " select id from ".USERS."  where group='User' order by id desc";
   		//$data['getCustomerDetails'] = $this->user_model->ExecuteQuery($Query);
		...
		$this->load->view('admin/users/customerExportExcel',$data);
	}
