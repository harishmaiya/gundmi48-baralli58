...
class Order_model extends My_Model
{
	...
	public function view_order_details($status){
		$this->db->select('p.*,u.email,u.firstname,u.address,u.phone_no,u.postal_code,u.state,u.country,u.city,pd.product_name,pd.id as PrdID,rq.totalAmt,rq.pro_totalAmt, rq.discount_amount,rq.pro_discount_amount, rq.Bookingno as bookingno');
	...
	public function view_order_detailsexcel($status=''){
		$this->db->select('p.*,u.email,u.firstname,u.address,u.phone_no,u.postal_code,u.state,u.country,u.city,pd.product_title,pd.id as PrdID,rq.Bookingno as bookingno');
		$this->db->from(PAYMENT.' as p');
		$this->db->join(USERS.' as u' , 'p.user_id = u.id');
		$this->db->join(PRODUCT.' as pd' , 'pd.id = p.product_id');		
		 $this->db->join(RENTALENQUIRY.' as rq' , 'rq.id = p.EnquiryId');
...
/*********************************************** Payment Success Cart********************************************************/	
	...
	public function PaymentSuccess($userid='', $randomId='' ,$transId = '', $payerMail = '',$type=''){
		...
		//Update Payment Table	
			...
			if($type!=''){
				...
				$payment_type='Authrozie Credit Payment';
				}
			}
			...
			if($payerMail != ''){
				...
				$dataArr1 = array('status' => 'Paid','shipping_status' => 'Processed', 'paypal_transaction_id' => $transId, 'payment_type' => $payment_type );
			}
...
$message.='</td>
   ...
   /* COnfirm booking for COD end */
	...
	public function get_enquiry_details($enqID)
	{
		$this->db->select('R.user_id, R.renter_id, R.Bookingno, U.firstname, U.lastname, U.email, U.phone_no, P.product_title, PA.address, U.image, PP.product_image');
