			...
			<div id="account">
				<div class="box">
					<div class="middle">
						...
						<h2><?php if($this->lang->line('PreviousTrips') != '') { echo stripslashes($this->lang->line('PreviousTrips')); } else echo "Previous Trips";?></h2>
						<div class="section notification_section">
							<div class="notification_area">
								...
								<form method="post" action="">
									...
									<input type="text" placeholder="Search your previous trips" name="product_title" value="<?php if(isset($_POST['product_title']))echo $_POST['product_title'];?>">
								...
								<tbody>
									<tr>
										...
										<td><img src="<?php if($row->product_image == ''){ echo base_url(). PRODUCTPATH.'dummyProductImage.jpg';  } else { if(strpos($row->product_image, 's3.amazonaws.com') > 1){ echo $row->product_image; }else {  if($row->product_image!= '' && file_exists('./server/php/rental/thumbnail/'.$row->product_image)){ echo base_url()."server/php/rental/thumbnail/".$row->product_image; } else {echo base_url()."server/php/rental/".$row->product_image;} } } ?>" width="100" height="100" alt="<?php echo $row->product_title; ?>"/> <a target="_blank" href="<?php echo base_url(); ?>rental/<?php echo $row->product_id; ?>" style="float:none;display:inline-block;"></br><?php echo $row->product_title; ?></a></td>
							...
							<?php if($loginCheck !=''){ echo form_open('site/product/add_dispute',array('id'=>'reviewForm1','style'=>'margin: 20px 20px 20px 40px'));?>
							...
							<textarea  name="message" id="review-text1" class="scroll_newdes" maxlength="300" style="height:90px; width: 440px;"></textarea>
							...
							<?php echo form_close(); } ?>
								...
								<?php } else { ?> <p><?php if($this->lang->line('Youhavenoprevious') != '') { echo stripslashes($this->lang->line('Youhavenoprevious')); } else echo "You have no previous trips.";?> <br><br></p><?php } ?>
...
<span class="johns" style="width:58%;"><b><?php echo $review['firstname']?></b></span>
</div>
...
$(document).ready(function(){
    $('.exemple5').jRating({
        ...
        decimalLength:1,
        onSuccess : function(){
            sweetAlert("Oops...", "Your rate has been saved", "success");
...
function add_review(){
	...
	var message_content = $('textarea#review-text').val();
