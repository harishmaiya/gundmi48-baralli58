...
// 			echo "<pre>";print_r($getLanguageContentDetails);die;
			//echo "<pre>";print_r($this->input->post('languageKeys'));die;
			/* file write start*/
			$loopItem = 0;
			$config = '<?php';
	...
	function change_multi_language_user_details()
	{
		$statusMode = $this->input->post('statusMode');
		$checkbox_id = $this->input->post('checkbox_id');
		if($statusMode != '' && !empty($checkbox_id))
		{
			$change_language_status = $this->multilanguage_model->change_language_user_status($statusMode,$checkbox_id);	
			$this->setErrorMessage('success'," Language settings changed successfully");
	...
	public function default_language() {
	 ...
	 $this->multilanguage_model->update_details(LANGUAGES,array('default_lang'=>""),array());
